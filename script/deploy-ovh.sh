...
echo "Keycloak:    https://$OVH_NIP_HOST/realms/lorrconnect | /realms/chabeille"

echo "✅ DONE - OVH cluster ready"

# STEP 6 - Setup lorrconnect Keycloak (idempotent, ne casse rien si déjà fait)
echo "🔐 STEP 6 - Setup lorrconnect Keycloak"
for i in $(seq 1 24); do
  STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:30090/realms/lorrconnect)
  if [ "$STATUS" = "200" ]; then
...
echo "✅ lorrconnect Keycloak configured!"
